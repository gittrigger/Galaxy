...
public class TabStart extends TabActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mLog.w(TAG, "onCreate() 84");
		{
			...
			t1.setIndicator(
					...
					getResources().getDrawable(
							android.R.drawable.ic_menu_search));// Custom.TOPICON
...

		{
			...
			// t.setIndicator(null,
			// getResources().getDrawable(R.drawable.briefcase));
			t.setIndicator(
					null,
					getResources().getDrawable(
...

		{
			...
			t.setIndicator(null, getResources()
					.getDrawable(R.drawable.outhouse));
			// t.setIndicator(null,
			// getResources().getDrawable(R.drawable.tab_out));
			t.setContent(listView2);
			mTabHost.addTab(t);
		}
		...
		// Thread getlatestThread = new Thread(){ public void run(){
		// getlatest(); } };
		...
		mTabHost.setOnTabChangedListener(new OnTabChangeListener() {
			...
			public void onTabChanged(String tabname) {
				int tab = mSharedPreferences.contains("tab") ? mSharedPreferences
						.getInt("tab", 0) : 0;
				int cur = mTabHost.getCurrentTab();
				mLog.w(TAG, "onTabChanced() arg0(" + tabname + ") currenttab("
						+ cur + ") lasttab(" + tab + ")");
		...
		Thread getloadRunner = new Thread() {
			public void run() {
				for (;;) {
					double load = mLog.getload(TAG
							+ " Custom(getloadRunner) 55");
					if (load < 4) {
		...
		boolean stoprequest = mIntentExtras != null ? mIntentExtras
				.getBoolean("stoprequest") : false;
		int tab = mSharedPreferences.contains("tab") ? mSharedPreferences
				.getInt("tab", 0) : 0;
		...
		if (stoprequest) {
			...
			// int total = mSharedPreferences.contains("total") ?
			// mSharedPreferences.getInt("total",-1) : -1;
			// if( total == -1 ){
			// Toast.makeText(this, "Loading Service for the First Time",
			// 5000).show();
			...
			PendingIntent service3 = PendingIntent.getBroadcast(this, 0,
					resetservice, Intent.FLAG_ACTIVITY_NEW_TASK
							| PendingIntent.FLAG_CANCEL_CURRENT);
			mAlM.set(AlarmManager.RTC_WAKEUP,
					(System.currentTimeMillis() + (2 * 1000)), service3);
	...
	protected void onStart() {
		...
		// service.putExtra("com.havenskys.newsbite.who", TAG +
		// " onStart() 214");
		// startService(service);

		/*
		 * / long syncstart = mSharedPreferences.getLong("syncstart", -1); long
		 * lastactive = mSharedPreferences.getLong("lastfeedactive", -1); long
		 * lowmemory = mSharedPreferences.getLong("lowmemory", -1);
		 * //mPreferencesEditor.putLong("lowmemory",
		 * System.currentTimeMillis()).commit();
		 * 
		 * if( syncstart < (System.currentTimeMillis() - 60 * 1000) && lowmemory
	...
	private Handler mTitleHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			String title = b.containsKey("title") ? b.getString("title") : "";
			String who = b.containsKey("who") ? b.getString("who") : "";

			TabStart.this.setTitle(title);
	...
	protected void onChildTitleChanged(Activity childActivity,
		...
		mLog.w(TAG, "onChildTitleChanged() tabCount("
				+ mTabHost.getTabWidget().getChildCount() + ") childActivity("
				+ childActivity.getClass().getName() + ") title(" + title
				+ ") ++++++++++++++++++++++++++++++++");
		...
		Bundle b = new Bundle();
		b.putString("who", "onChildTitleChanged");
		b.putString("title", title.toString());
		msg.setData(b);
		mTitleHandler.sendMessageDelayed(msg, new Long(100));
		if (name.contains("browseView")) {
			if (title.length() == 0) {
				// mTabHost.requestFocusFromTouch();
				int lasttab = mSharedPreferences.contains("lasttab") ? mSharedPreferences
						.getInt("lasttab", 0) : 0;
				if (lasttab == 1) {
					lasttab = 0;
				}
		...
		if (name.contains("listView") || name.contains("listSave")
				|| name.contains("listOut")) {
			...
			if (id > 0) {
				...
				/*
				 * int topimage = android.R.drawable.ic_menu_compass; int feedid
				 * = 0; Cursor dataCursor = SqliteWrapper.query(this,
				 * this.getContentResolver(), DataProvider.CONTENT_URI ,new
				 * String[]{"type"} ,"_id = " + id ,null ,null //orderby
				 * //"date desc" );
				 * 
				 * if( dataCursor != null ){ if( dataCursor.moveToFirst() ){
				 * feedid = dataCursor.getInt(0); topimage =
				 * Integer.parseInt(mLog.dataFeed[feedid][2]); } }
				 * 
				 * if( mTabHost.getChildCount() > 1 ){ TabWidget tw =
				 * mTabHost.getTabWidget(); tw.removeViewAt(1); }
				 * 
				 * Intent browseView = new Intent(this, browseView.class);
				 * //browseView.putExtra("id", id); TabSpec t2 =
				 * mTabHost.newTabSpec("browse"); t2.setIndicator(null,
				 * getResources().getDrawable(topimage));
				 * t2.setContent(browseView); mTabHost.addTab(t2);
				 * 
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		// "View Source Webpage").setIcon(android.R.drawable.ic_menu_view);

		// menu.add(Menu.NONE, 202, 1, "Email " +
		// mLog.WHO).setIcon(Custom.LITTLEICON);

		menu.add(Menu.NONE, 1, 2, "Doc Chomps Software").setIcon(
				R.drawable.docdot);

		menu.add(Menu.NONE, 2, 3, "Help").setIcon(
				android.R.drawable.ic_menu_help);

		menu.add(Menu.NONE, 201, 4, "Email Support").setIcon(
				android.R.drawable.ic_menu_send);

		menu.add(Menu.NONE, 202, 5, "Configure Login").setIcon(
				android.R.drawable.ic_menu_preferences);

		{
			int groupNum = 31;
			SubMenu submenu = menu.addSubMenu(Menu.NONE, groupNum, 6,
					"Configure Sync");
			submenu.setIcon(android.R.drawable.ic_menu_preferences);
			submenu.add(groupNum, 0, 0, "No");
			submenu.add(groupNum, 1, 2, "Yes");
			boolean syncandroid = mSharedPreferences.getBoolean("syncandroid",
					false);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		mLog.w(TAG, "onOptionsItemSelected() groupid(" + item.getGroupId()
				+ ") itemid(" + item.getItemId() + ") title(" + item.getTitle()
				+ ")");
		...
		case 201: {
			...
			jump.putExtra(
					Intent.EXTRA_TEXT,
					"This is a request for special help.  Contained in this request are the only details to help diagnose and understand an issue.\n\n\n\n");
			jump.putExtra(Intent.EXTRA_EMAIL, new String[] { "\"" + Custom.APP
		...
		case 202: {
			...
			// jump.putExtra(Intent.EXTRA_EMAIL, new String[] {"\""+Custom.WHO +
			// "\" <"+Custom.EMAIL+">"});
	...
	private void serviceRestart(final String who) {
		...
		// mPreferencesEditor.putBoolean("servicerestart", true);
		// mPreferencesEditor.commit();
