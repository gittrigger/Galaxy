...
public class SyncProcessing {
	...
	public void setSharedPreferences(SharedPreferences sharedPreferences){
		...
		long lasthttp = mSharedPreferences.getLong("lasthttp", 0);
		if( lasthttp > 0 ){
			String cshort = "";
			if( lasthttp > System.currentTimeMillis() - 6000000 ){
				 cshort = mSharedPreferences.getString("lastcookies","");
				 ...
				 mUrl = webaddress;
			    	...
			    	mDestination = mSharedPreferences.contains("destination") ? mSharedPreferences.getString("destination", "") : "";
			        mPostpath = mSharedPreferences.contains("postpath") ? mSharedPreferences.getString("postpath", "") : "";
			...
			for(int h=0; h < clist.length; h++){
				...
				Log.w(TAG,"Recreate cookie " + c[0] + ":"+c[1]);
			...
			
			{
	        	...
	        	for(int i = 0; i < cl2.size(); i++){
	        		Cookie c3 = cl2.get(i);
	        		Log.w(TAG, "setSharedPreferences() 113 "+c3.getName() + " " + c3.getValue() + " (" + c3.getDomain()+" p" + c3.isPersistent()+" s" + c3.isSecure() +" " + c3.getPath() +")");
	        		//cshort += c3.getName() +" " + c3.getValue() +"\n";
	        	}
	        	//mPreferencesEditor.putLong("lasthttp",System.currentTimeMillis());
	        	//mPreferencesEditor.putString("lastcookies", cshort);mPreferencesEditor.commit();
	        }
			}
		}
	}
	...
	public int owaLogin(String who, String url, String login, String password) {
        ...
        if( !httpStatus.contains("200") ){
        	...
        	for(int i = 0; i < cl2.size(); i++){
        		Cookie c3 = cl2.get(i);
        		Log.w(TAG, "owaLogin() 305 "+c3.getName() + " " + c3.getValue() + " (" + c3.getDomain()+" p" + c3.isPersistent()+" s" + c3.isSecure() +" " + c3.getPath()+" " +c3.getVersion() +") for "+who);
        		cshort += c3.getName() +" " + c3.getValue() +"\n";
        	}
        	mPreferencesEditor.putLong("lasthttp",System.currentTimeMillis());
        	mPreferencesEditor.putString("lastcookies", cshort);mPreferencesEditor.commit();
